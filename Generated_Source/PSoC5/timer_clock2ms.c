...
/*******************************************************************************
* Function Name: timer_clock2ms_Start
********************************************************************************
*
* Summary:
...
void timer_clock2ms_Start(void) 
{
    ...
    timer_clock2ms_CLKEN |= timer_clock2ms_CLKEN_MASK;
	timer_clock2ms_CLKSTBY |= timer_clock2ms_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: timer_clock2ms_Stop
********************************************************************************
*
* Summary:
...
void timer_clock2ms_Stop(void) 
{
    ...
    timer_clock2ms_CLKEN &= (uint8)(~timer_clock2ms_CLKEN_MASK);
	timer_clock2ms_CLKSTBY &= (uint8)(~timer_clock2ms_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: timer_clock2ms_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((timer_clock2ms_CLKEN & timer_clock2ms_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = timer_clock2ms_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = timer_clock2ms_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* timer_clock2ms__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(timer_clock2ms_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(timer_clock2ms_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: timer_clock2ms_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: timer_clock2ms_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = timer_clock2ms_GetSourceRegister();
    uint16 oldDivider = timer_clock2ms_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = timer_clock2ms_CLKEN & timer_clock2ms_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(timer_clock2ms_DIV_PTR, clkDivider);
                timer_clock2ms_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                timer_clock2ms_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(timer_clock2ms_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((timer_clock2ms_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: timer_clock2ms_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(timer_clock2ms_DIV_PTR);
}


/*******************************************************************************
* Function Name: timer_clock2ms_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock2ms_MOD_SRC |= modeBitMask & (uint8)timer_clock2ms_MODE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock2ms_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock2ms_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(timer_clock2ms_MODE_MASK));
}


/*******************************************************************************
* Function Name: timer_clock2ms_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return timer_clock2ms_MOD_SRC & (uint8)(timer_clock2ms_MODE_MASK);
}


/*******************************************************************************
* Function Name: timer_clock2ms_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = timer_clock2ms_GetDividerRegister();
    uint8 oldSrc = timer_clock2ms_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        timer_clock2ms_MOD_SRC |= CYCLK_SSS;
        timer_clock2ms_MOD_SRC =
            (timer_clock2ms_MOD_SRC & (uint8)(~timer_clock2ms_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: timer_clock2ms_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return timer_clock2ms_MOD_SRC & timer_clock2ms_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: timer_clock2ms_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock2ms_PHASE = clkPhase & timer_clock2ms_PHASE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock2ms_GetPhase
********************************************************************************
*
* Summary:
...
{
    return timer_clock2ms_PHASE & timer_clock2ms_PHASE_MASK;
}
